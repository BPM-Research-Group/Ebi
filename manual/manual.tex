	...
	\label{sec:speedtricks}
		\begin{enumerate}
			...
			The results are then not guaranteed, but it may be faster.
   			...
   			\item Ebi can be compiled without exact/approximate arithmetic, using the \verb=withoutexactarithmetic=/\verb=withoutapproximatearithmetic= features, which is faster than the default compilation in which the user chooses it using the \verb=-a= parameter.
            		When Ebi is compiled with any of this features, the \verb=-a= parameter has no effect.
		\end{enumerate}
