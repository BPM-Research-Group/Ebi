		...
		\subsubsection{The command}
			...
			\begin{enumerate}
				...
				\begin{itemize}
					...
					Notice that the link between \verb=input_types= and \verb=execute= cannot be verified at compile time, thus this should be tested extensively.
                    The framework performs a one-off test in some cases, but cannot be exhaustive (see Section~\ref{sec:tests}).
					...
					Notice that the link between \verb=execute= and \verb=output_type= cannot be verified at compile time, thus this should be tested extensively.
                    The framework performs a one-off test in some cases, but cannot be exhaustive (see Section~\ref{sec:tests}).
...
\end{lstlisting}
        ...
        \label{sec:tests}
            ...
            \end{itemize}
            Note that only a single attempt is made at calling each command: the files in the \verb=testfiles= are searched in arbitrary order until a combination is found that should enable the command.
            For performance reasons, no further applicable combinations of files are sought.

            Further tests can (and should) be located at the bottom of the source file, following Rust conventions.
            For instance:
\begin{lstlisting}[language=Rust, style=boxed]
#[cfg(test)]
mod tests {
