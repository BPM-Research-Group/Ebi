...
\author{The BPM group at RWTH Aachen University and contributors\\
Sander J.J. Leemans, Tian Li, Leonhard Mühlmeyer, Philipp Moch, Adam Burke, Renas Demir, Felix Britzelmaier, Levi Heiland}
\date{\today}

\DeclareMathOperator{\logdiv}{logdiv}
\DeclareMathOperator{\Logdiv}{Logdiv}
\DeclareMathOperator{\logpol}{logpol}
...
\chapter{Ebi: introduction}
	...
	Ebi has \numberofcommands{} commands; for an overview of commands please refer to \cref{sec:commands} or give the command-line parameter \verb=--help=.
	\numberoffilehandlers{} different types of files are supported; \cref{sec:filehandlers} contains an overview of the files that Ebi supports.
	
	The architecture of Ebi and how to develop new commands can be found in \cref{sec:architecture}.
    ...
    \section{Python/PM4Py plug-in}
        ...
        \subsection*{Installation}
            ...
            %Alternatively, Ebi can be installed in Python manually:
            %first set up Ebi for local development,
            %install Maturin, and generate a .whl file using \verb=maturin build --features python=.
            %Finally, install the Ebi python package using \verb=pip install ⟨file name⟩.whl -force-reinstall=.

        \subsection*{Usage}
            Every command can be used as a Python plug-in; the function name can be found with the respective command in \cref{sec:commands}.
...
print(result)\end{lstlisting}
            ...
            All other Ebi inputs and outputs are treated as strings.
            The inputs of a command function in Python are the mandatory parameters, indicated by $\langle$ $\rangle$ at the command in this manual.
            The optional parameters cannot be given from Python, and neither can the output format be selected.

    \section{Rust crate}
                Ebi can be used as a Rust crate, available on \href{https://crates.io/crates/ebi}{crates.io}.

                To add it a Rust project, add the following line to the \texttt{cargo.toml} file in the \texttt{[dependencies]} section:
                \begin{lstlisting}[language=rust, style=boxed]
ebi = "*"\end{lstlisting}

                It may be necessary to Box an object and to cast it to a trait in order to call certain commands on it (due to the runtime type checking that Ebi performs). 
                For instance:
                \begin{lstlisting}[language=rust, style=boxed]
use std::fs;
...
\label{sec:commands}
	...
	This chapter lists the commands supported by Ebi.

    \ebicommands

\chapter{Supported files}
\label{sec:filehandlers}
	This chapter lists the file types that are supported by Ebi, and for each file type which commands can use it as input or produce it as output.
...
\label{sec:architecture}
    ...
    % \restoregeometry
        ...
        For the commands of Ebi, please refer to \cref{sec:commands}.
    ...
    \label{sec:objecttypes}
        ...
        Ebi has the following object types; please refer to the \href{https://docs.rs/ebi_objects/latest/ebi_objects/}{documentation} for more information.
        \ebiobjecttypelist
    
    \section{File handlers}
        ...
        \subsection{File Handlers $\rightarrow$ Objects (importers)}
            \begin{center}
                \filehandlersobjecttypes
            \end{center}
            In this table, \CIRCLE{} means that the export is always possible, and \LEFTcircle{} means that a conversion is involved that may fail.

        \clearpage
        \subsection{File Handlers $\rightarrow$ Traits (importers)}
            \begin{center}
                \filehandlerstraits
            \end{center}
        
        \clearpage
        \subsection{Objects $\rightarrow$ file handlers (exporters)}
            \begin{center}
                \objecttypesfilehandlers
            \end{center}
            In this table, \CIRCLE{} means that the export is always possible, and \LEFTcircle{} means that a conversion is involved that may fail.
        \restoregeometry
            
	\section{Implementing a new command/algorithm/technique}
	\label{sec:implement}
		...
		\subsection{Conventions and Hints}
			\begin{enumerate}
				\item Ebi only uses floating point numbers (f64) when the user asks for it.
				Commands should use \verb=Fraction= or other exact numeric classes (see \cref{sec:exact}); these perform computations with the arithmetic type selected by the user.
...
\end{lstlisting}
		...
		\subsection{The technique}
			...
			\begin{enumerate}
				...
				See \cref{sec:traits} for an overview of Ebi traits.
				Alternatively, object types can be used as inputs; please see \cref{sec:objecttypes} for an overview of available object types.
				...
				Ebi deliberately does not support floating point numbers (f32, f64) as these are not exact (see \cref{sec:exact}).
		...
		\subsection{The command}
			...
			\begin{enumerate}
				...
				\begin{itemize}
					...
					Ebi tests the \verb=execute= function automatally, see \cref{sec:tests}.
                    ...
                    Ebi automatally tests whether the output of \verb=execute= matches the declared \verb=output_type=; see \cref{sec:tests}.
...
\end{lstlisting}
        ...
        \subsection{Code structure}
            Ebi is divided over several Rust crates:
            \begin{itemize}
                \item Ebi is the main crate, and contains the commands, techniques, CLI, and links with other programming languages.
                Furthermore, in this crate the framework can be found, that is, the enum declarations of commands, file handlers and traits.
                \item Ebi\_objects contains all code for objects, including input, output and conversions between objects.
                \item Ebi\_bpmn contains the code for stochastic and non-stochastic BPMN models: parser, exporter and semantics.
                \item Ebi\_activity\_key contains the code for activity keys, which de-duplicate strings.
                \item Ebi\_arithmetic contains math-based code, such as fractions.
                \item Ebi\_optimisation contains optimisers.
                \item Ebi\_derive contains derivation macros.
            \end{itemize}
...
\end{lstlisting}
    ...
    \label{sec:exact}
        ...
        \subsection{LogPolynomial}
            Logarithms can often not be expressed in $\mathbb{Q}$.
            For instance, $\log_2(3) \notin \mathbb{Q}$ is not a rational number.
            To perform computations in an exact manner, a log polynomial is a list of terms 
            $\logpol((q_0, n_0) \ldots (q_m, n_m))$ with $q_i \in \mathbb{Q}, q_i \neq 0$ and $n_i \in \mathbb{P}$ (prime numbers), and each $n_i$ is unique.
            The value of such a log polynomial is:
            $$\logpol((q_0, n_0) \ldots (q_m, n_m)) = \sum_i q_i \log_2 n_i$$

            The value zero is:
            $$ 0 = \logpol()$$

            The value of any fraction $q \in \mathbb{Q}$ is:
            $$ q = \logpol((q, 2))$$

            A basic operation, but also an expensive operation, is to take the logarithm of a natural number $n \in N, n > 0$.
            This step requires the prime factorisation of $n$, which may be infeasible for very large n\footnote{However, this is in most cases preferred over a LogDiv, which has to compute large powers during most operations, while LogPolynomial only computes the prime factorisation while taking the logarithm.}.
            $$ \log_2(n) = \logpol((q_1, p_1) \ldots (q_m, p_m)) \text{ with } n = p_1^{q_1} * \ldots * p_m^{q_m}$$
            
            To take the logarithm of a fraction $q \in Q$, we simply use the rule that $\log_2(\frac{a}{b}) = \log_2(a) - \log_2(b)$.

            Summing and subtraction of log polynomials is cheap as one simply sums or subtracts the $q_i$s of corresponding terms.
            Multiplication and division of a log polynomial with a fraction distributes over the $q_i$s.

            Multiplication and division of log polynomials with one another is not supported.

            To approximate a log polynomial, the individual terms are approximated and summed.

            A log polynomial can be collapsed to a single term $(q, n)$:
            \begin{align*}
                \logpol((\frac{a}{b}, c), (\frac{d}{e}, f)) =& \frac{a \log_2(c)}{b} + \frac{d \log_2(f)}{e}\\
                & \frac{ea \log_2(c)}{be} + \frac{bd \log_2(f)}{be}\\
                & \frac{\log_2(c^{ea}) + \log_2(f^{bd})}{be}\\
                & \frac{\log_2(c^{ea} f^{bd})}{be}\\
                & \frac{1}{be} \log_2(c^{ea} f^{bd})
            \end{align*}
            Experience has shown that such a single term quickly gets uncomprehensibly and unfeasibly large.
