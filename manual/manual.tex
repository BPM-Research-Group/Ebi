...
\section{Ebi: introduction}
	...
	\label{sec:speedtricks}
		\begin{enumerate}
            ...
            Another option is to compile Ebi without exact arithmetic, using the \verb=withoutexactarithmetic= feature, which may lead to a speadup in comparison with the \verb=-a= parameter.
            When this feature is given, the \verb=-a= parameter has no effect.

            Yet another option is to compile Ebi without approximate arithmetic, using the \verb=withoutapproximatearithmetic= features, which will have a speed advantage, but disables the \verb=-a= parameter.
