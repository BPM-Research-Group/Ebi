...
pub const EBI_PROBABILITY_EXPLAIN_LOG_WITHOUT_H: EbiCommand = EbiCommand::Command { 
    ...
    execute: |mut inputs, cli_matches| {
        ...
        save_vectors_to_csv(&trace_len_vec, &time_vec, &mem_vec, &states_vec, balance.fraction_to_f64().unwrap(),"data/road_id02_a=0.5_noheuristic.csv");
        Ok(EbiOutput::Fraction(Fraction::zero()))
    }, 
    output_type: &EbiOutputType::Fraction
};
...
fn save_vectors_to_csv<T: std::fmt::Display>(
    ...
    writer.write_record(&["Trace length", "Time", "Memory", "States", "Cost", "Probability", "Balance"])?;

    // Iterate through the vectors and write rows
    for i in 0..vec1.len().min(vec2.len()).min(vec3.len()) {
        writer.write_record(&[
            vec1[i].to_string(),
            vec2[i].to_string(),
            vec3[i].to_string(),
