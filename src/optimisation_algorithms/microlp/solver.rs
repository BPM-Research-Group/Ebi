...
const MACHINE_EPS: f64 = f64::EPSILON * 10.0;
...
mod tests {
    ...
    fn to_sparse(dense: &[f64]) -> CsVec<f64> {
        let mut triplets = vec![];
        for (i, &val) in dense.iter().enumerate() {
            if val != 0.0 {
                triplets.push((0, i, val));
            }
        }
        let mut mat = TriMat::with_capacity((1, dense.len()), triplets.len());
        for (r, c, val) in triplets {
            mat.add_triplet(r, c, val);
        }
        mat.to_csc::<usize>().outer_view(0).unwrap().to_owned()
    }
    ...
    fn assert_matrix_eq(actual: &CsMat, expected: &[Vec<f64>]) {
        ...
        for (r, expected_row) in expected.iter().enumerate() {
            ...
            assert_eq!(actual_row, *expected_row, "Row {} differs", r);
        }
    }
    ...
    fn initial_solve() {
        ...
        let mut sol = Solver::try_new(
            &[3.0, 4.0],  // Positive coefficients for minimization
            &[0.0, 5.0],  // x0 >= 0, x1 >= 5
            &[20.0, 20.0], // x0 <= 20, x1 <= 20 (make both bounded)
            &[
