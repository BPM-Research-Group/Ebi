use std::{collections::BTreeSet, fmt::{self, Debug, Display, Pointer}, fs::File, hash::Hash, io::Write, path::PathBuf};

use anyhow::{Result, Context};

use clap::ArgMatches;

use crate::{ebi_commands::ebi_command::{EbiCommand, EBI_COMMANDS}, ebi_objects::ebi_object::{EbiObject, EbiObjectType}, file_handler::{self, EbiFileHandler, EBI_FILE_HANDLERS}, math::{fraction::Fraction, log_div::LogDiv, root::ContainsRoot, root_log_div::RootLogDiv}};
...
impl EbiExporter {
    pub fn export_from_object(&self, output: EbiOutput, f: &mut dyn std::io::Write) -> Result<()> {
        match (self, output) {
            ...
            (EbiExporter::Object(_, _), _) => unreachable!(),
