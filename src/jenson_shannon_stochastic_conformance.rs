...
pub fn jssc_log2log(event_log1: Box<dyn EbiTraitFiniteStochasticLanguage>, event_log2: Box<dyn EbiTraitFiniteStochasticLanguage>) -> Result<RootLogDiv> {
    ...
    for (trace1, probability1) in event_log1.iter_trace_probability() {
    ...
    // return Ok(LogDiv::from(Fraction::one() - (LogDiv::approximate(&sum).unwrap()/2).sqrt_abs(10)));
}

pub fn jssc_log2model(log1: Box<dyn EbiTraitFiniteStochasticLanguage>, mut logmodel2: Box<dyn EbiTraitQueriableStochasticLanguage>) -> Result<RootLogDiv> {
    let mut sum = LogDiv::zero();
