use crate::{
    ...
    math::{astar, fraction::Fraction, traits::{One, Zero}},
...
impl<State: Displayable>
    dyn StochasticSemantics<StoSemState = State, SemState = State, AliState = State>
{
                ...
                if let Some(activity) = self.get_transition_activity(transition) {
                    ...
                    result.push((
                        ...
                        StochasticWeightedCost {
                            ...
                            stochastic_weighted_cost: &transition_probability.clone().one_minus()
                                * 10000,
                        },
                    ));
