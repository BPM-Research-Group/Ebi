...
impl SPNMiner for dyn EbiTraitFiniteStochasticLanguage {
    ...
    fn mine_stochastic_petri_net(&self) -> grb::Result<StochasticLabelledPetriNet> {
        ...
        for max_places in 3..10 {
            // Setup - adjusted bounds for correct multiset combinations
            let max_init_tokens = 100;
            let max_arc_weight = 100; // Allow weights greater than 1
            let time_limit = 60.0 * max_places as f64; 
            let weight_lb = 0.00001; // MUST be strictly > 0 to avoid breaking bilinear constraints
            let weight_ub = 1.0;
            let mip_gap = 0.000000001;
            ...
            for t in 0..trans_num {
                ...
                println!("Transition {} weight: {:.6}", t, sol_w[t]);
            }
            ...
            println!("activity key in spn miner: {:?}", slpn.activity_key);
            ...
            for (new_p, &orig_p) in kept_places.iter().enumerate() {
                for t in 0..trans_num {
                    ...
                    if in_w > 0 { let _ = slpn.add_place_transition_arc(new_p, t, in_w); }

                    let out_w = sol_out[orig_p][t];
                    if out_w > 0 { let _ = slpn.add_transition_place_arc(t, new_p, out_w); }
                }
            }

            let uemsc_res = self.unit_earth_movers_stochastic_conformance(Box::new(slpn.clone())).unwrap();
            println!("current uemsc: {:.6}", uemsc_res.clone());
