...
macro_rules! align_for_lpn {
    ($t:ident) => {
        ...
        impl $t {
            pub fn align_astar(&self, trace: &Vec<Activity>) -> Option<(Vec<(usize, LPNMarking)>, f64)> {
                let start = (0, self.get_initial_state());
                let successors = |(trace_index, state) : &(usize, LPNMarking)| {
            
                    let mut result: Vec<((usize, LPNMarking), (usize, TransitionIndex), f64)> = vec![];
                    ...
                    if trace_index < &trace.len() {
                        ...
                        result.push(((new_trace_index, new_state), (*trace_index+1, 0), 10000.0));
                    }
                    ...
                    for transition in self.get_enabled_transitions(&state) {
                        ...
                        if let Some(activity) = self.get_transition_activity(transition) {
                            ...
                            result.push(((*trace_index, new_state.clone()), (0, transition+1), 10000.0));
                            ...
                            result.push(((*trace_index, new_state), (0, transition+1), 1.0));
                        }
                    }
                ...
                let result_cost_function: (Vec<f64>, HashMap<(usize, usize), usize>) = self.get_move_cost_function(trace);
                let cost_function = result_cost_function.0;
                let cost_function_map = result_cost_function.1;
                let temp_result = self.get_incidence_matrix(trace);
                ...
                let heuristic = |(trace_index, state) : &(usize, LPNMarking)| {
                    let marking_vec = self.get_marking_vector(trace, trace_index, state);
                    // get the index of the transition
                    let result = match self.underestimate_move_cost(&marking_vec, 
                        &cost_function, 
                        &incidence_matrix, 
                        &transition_preset){
                ...
                let success = |(trace_index, state): &(usize, LPNMarking)| {
                ...
                fn get_marking_vector(&self, trace: &Vec<Activity>, trace_index: &usize, state: &LPNMarking) -> Vec<f64> {
                ...
                fn get_move_cost_function(&self, trace:&Vec<Activity> ) -> (Vec<f64>,HashMap<(usize,TransitionIndex), usize>){
                    ...
                    for model_transition in 0..self.get_number_of_transitions() {
                        ...
                        if self.is_transition_silent(model_transition){
                            ...
                            move_cost_function.push(100000.0);
                            transition2cost.insert((0,model_transition+1), transition_count);
                            transition_count += 1;
                        }
                    }
        ...
        impl Align for $t {
            ...
            fn align_trace(&self, trace: &Vec<Activity>) -> Result<(Vec<Move>, usize)> {
                if let Some((states, cost)) = self.align_astar(trace) {
                    ...
                    Err(anyhow!("It is impossible to reach a deadlock."))
                }
            }
        }
        impl $t {
            pub fn align_astar(&self, trace: &Vec<Activity>) -> Option<(Vec<(usize, $s)>, usize)>  {
  
                let start = (0, self.get_initial_state());
                let successors = |(trace_index, state) : &(usize, $s)| {
                ...
                let heuristic = |(_trace_index, _state) : &(usize, $s)| {
                    0
                };
            
                //function that returns whether we are in a final synchronous product state
                let success = |(trace_index, state): &(usize, $s)| {
                    let result = trace_index == &trace.len() && self.is_final_state(&state);
                    // log::debug!("state {} {} is final: {}", trace_index, state, result);
                    result
                };
