...
use ebi_objects::ebi_arithmetic::{fraction::{approximate::Approximate, fraction_enum::FractionEnum}};
...
pub trait SPNMiner {
     fn mine_stochastic_petri_net(&self, place_num: usize) -> grb::Result<StochasticLabelledPetriNet>;
}
...
impl SPNMiner for dyn EbiTraitFiniteStochasticLanguage {
    ...
    fn mine_stochastic_petri_net(&self, place_num: usize) -> grb::Result<StochasticLabelledPetriNet> {
        ...
        let targer_place_num = place_num;
        ...
        let n_places = if targer_place_num > 0 { targer_place_num } else { 2 * trans_num };
        ...
        model.set_param(param::TimeLimit, 86400.0)?;
        model.set_param(param::MIPFocus, 1)?;
        ...
        println!("current {:.6} with predefined place number: {}", uemsc_res, targer_place_num);
