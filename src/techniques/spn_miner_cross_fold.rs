...
pub trait SPNMinerCrossFold {
     fn mine_spn(&self, test_slang: Box<dyn EbiTraitFiniteStochasticLanguage>) -> grb::Result<StochasticLabelledPetriNet>;
}

impl SPNMinerCrossFold for dyn EbiTraitFiniteStochasticLanguage {
    fn mine_spn(&self, test_slang: Box<dyn EbiTraitFiniteStochasticLanguage>) -> grb::Result<StochasticLabelledPetriNet> {
        let mut best_spn = StochasticLabelledPetriNet::new();
        let mut best_uemsc = Fraction::zero();
        let mut best_obj = 1.0;
        let mut wtr = Writer::from_path("./testfiles/road_fold1_results.csv").unwrap();

        for max_places in 1..35{
            // Setup - adjusted bounds for correct multiset combinations
            let max_init_tokens = 16;
            let max_arc_weight = 16;
            let time_limit = 600.0 * max_places as f64; 
            ...
            // 2. Set the Cutoff parameter directly on the model
            model.set_param(param::Cutoff, best_obj)?;
            ...
            // model.add_constr(&format!("obj_constr"), c!(obj <= best_obj))?;
            model.optimize()?;
            ...
            let uemsc_res = test_slang.unit_earth_movers_stochastic_conformance(Box::new(slpn.clone())).unwrap();

            println!("current {:.6} with predefined place number: {}", uemsc_res, max_places);

            wtr.serialize(Record {
                max_places,
                time_secs: time_limit,
                status: status as i32,
                objective: uemsc_res.clone().approximate().unwrap(),
            }).unwrap();
            wtr.flush().unwrap();
