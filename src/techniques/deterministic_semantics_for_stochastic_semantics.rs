...
impl <T, FS> StochasticDeterministicSemantics for T where T: StochasticSemantics<AState = FS, State = FS>, FS: Displayable {
// impl <S: Displayable> StochasticDeterministicSemantics for DeterministicStochasticSemantics<S> {
    type DState = PMarking<LPNMarking>;

    fn get_activity_key(&self) -> &ActivityKey {
        <T as Semantics>::get_activity_key(&self)
    }
...
impl <FS> dyn StochasticSemantics<AState = FS, State = FS> where FS: Displayable {
    /**
     * Compute the next p-state.
     */
    fn compute_next(&self, p_state: &mut PMarking<FS>) -> Result<()> {
     ...
     */
    fn markov_markings(&self, p_state: &PMarking<FS>) -> Result<(MarkovMarkings<FS>, Matrix)> {
    ...
    fn markov_matrices(&self, markov_markings: &MarkovMarkings<FS>) -> Result<(Matrix, Matrix)> {
            ...
            match markov_marking {
                MarkovMarking::Transient(marking) => { //a transient marking goes to B
                    ...
                    for transition in self.get_enabled_transitions(marking) {
                        ...
                        let probability = self.get_transition_weight(marking, transition) / &total_weight;
