...
impl StochasticLabelledPetriNet {
    ...
    fn create_markov_model(
        &self,
        q_state: &PMarking<LPNMarking>,
    ) -> Result<MarkovModel<MarkovMarking<LPNMarking>>> {
            ...
            for transition in self.get_enabled_transitions(&marking) {
                ...
                if self.is_transition_silent(transition) {
                    ...
                    if self.is_final_state(&new_marking) {
                        ...
                        let (new_markov_index, _) = markov
                            .add_or_find_state(MarkovMarking::Final(new_marking), Fraction::zero());
                        ...
                        let new_markov_marking =
                            MarkovMarking::ReachableWithSilentTransitions(new_marking.clone());
