 ...
 */
pub fn log_categorical_attribute(event_log: &Box<dyn EbiTraitEventLog>, number_of_samples: usize, trace_attribute: &String, alpha: &Fraction) -> Result<(Fraction, bool)> {

    let mut attribute_key = AttributeKey::new();
    let traces_with_attributes = event_log.get_traces_with_categorical_attributes(&mut attribute_key, trace_attribute).into_iter().collect::<Vec<_>>();
    ...
    let mut trace_indices = Vec::with_capacity(event_log.len());
    let mut attribute_indices = Vec::with_capacity(event_log.len());
    ...
    let e: usize = (0..number_of_samples).into_iter().map(|s| {
        ...
        let mut sample = vec![0; event_log.len()];
        sample::sample_indices(event_log.len(), &mut sample);
