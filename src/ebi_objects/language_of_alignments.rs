...
pub const EBI_LANGUAGE_OF_ALIGNMENTS: EbiFileHandler = EbiFileHandler {
    ...
    object_importers: &[
        EbiObjectImporter::LanguageOfAlignments(LanguageOfAlignments::import_as_object)
    ],
    object_exporters: &[ 
        EbiObjectExporter::LanguageOfAlignments(LanguageOfAlignments::export_from_object),
        EbiObjectExporter::StochasticLanguageOfAlignments(LanguageOfAlignments::export_from_stochastic_language_of_alignments),
    ],
...
impl LanguageOfAlignments {
    ...
    pub fn export_from_stochastic_language_of_alignments(object: EbiOutput, f: &mut dyn std::io::Write) -> Result<()> {
        match object {
            EbiOutput::Object(EbiObject::StochasticLanguageOfAlignments(slang)) => {
                let lang: LanguageOfAlignments = slang.into();
                lang.export(f)
            },
            _ => unreachable!()
        }
